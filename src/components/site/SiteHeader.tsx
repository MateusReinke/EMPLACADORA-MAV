...
export const SiteHeader = () => {
  ...
  return (
    <header className="sticky top-0 z-40">
      ...
      <div className="hidden bg-mav-navy text-white lg:block">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-6 px-6 py-2 text-[0.8125rem]">
          <p className="flex items-center gap-2 text-white/80">
            <MapPin className="h-3.5 w-3.5 text-mav-blue" strokeWidth={2.5} />
      ...
      <div
        className={`border-b bg-white transition-shadow duration-200 ${
          scrolled ? "border-mav-line shadow-[0_10px_30px_-24px_rgba(10,31,68,0.6)]" : "border-transparent"
        }`}
      >
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-5 py-3 sm:px-6">
          ...
          <nav className="hidden items-center gap-1 lg:flex" aria-label="Navegação principal">
            {NAV_ITEMS.map((item) => (
              <a
                key={item.href}
                href={item.href}
                className="rounded-md px-3 py-2 font-display text-sm font-semibold text-mav-navy/75 transition-colors hover:bg-mav-blue-soft hover:text-mav-blue"
              >
                {item.label}
              </a>
            ))}
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex h-11 w-11 items-center justify-center rounded-lg border border-mav-line text-mav-navy transition-colors hover:bg-mav-surface lg:hidden"
        ...
        {menuOpen && (
          <div id="menu-mobile" className="border-t border-mav-line bg-white px-5 pb-5 pt-2 lg:hidden">
            <nav className="flex flex-col" aria-label="Navegação principal (mobile)">
              {NAV_ITEMS.map((item) => (
                <a
                  ...
                  className="border-b border-mav-line/70 py-3 font-display text-base font-semibold text-mav-navy last:border-0"
                >
                  {item.label}
                </a>
              ))}
            ...
            <a
              href={`tel:${BUSINESS.phoneE164}`}
              className="mt-4 flex items-center justify-center gap-2 rounded-lg border border-mav-line py-3 font-display font-bold text-mav-navy"
            >
              <Phone className="h-4 w-4 text-mav-blue" strokeWidth={2.5} />
              {BUSINESS.phoneDisplay}
            </a>
          </div>
        )}
